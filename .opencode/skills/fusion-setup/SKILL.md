...
- Claude Code bridge: copy `<this-skill-dir>/plugins/fusion-claude.js` -> `~/.config/opencode/plugins/fusion-claude.js`. Also merge `"fusion_claude_*": "deny"` into the top-level `permission` map. The build and plan agent files contain the two exact allows that override this global deny. If the existing top-level permission is shorthand, preserve it as `"*"` before adding the Claude deny. This bridge is deliberately review-only and optional. Note that re-running the installer without the `claude` extra does not remove a previously installed bridge - the plugin file, the global deny, and the build/plan allows all stay in place; removal is `install.js undo` (or deleting the plugin file and the deny by hand).
